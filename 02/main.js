...
const points = (first, second) => {
    ...
    if ((move === 'rock' && firstMove === 'scissors') ||
...
function calculatePoints(firstColumn, secondColumn) {
    ...
    let secondIndex = increaseMove + firstIndex;
    if (secondIndex > moves.length - 1)
        secondIndex = 0;
    if (secondIndex === -1)
        secondIndex = moves.length - 1;
    const secondMove = moves[secondIndex];
    const score = scores[secondIndex];
...
const countPoints = textArray.reduce((acc, elm) => {
    const columns = elm.split(' ');
    const score = points(columns[0], columns[1]);
    return acc + score;
}, 0);
const secondStrategy = textArray.reduce((acc, elm) => {
    const columns = elm.split(' ');
